...
var QUnit = {
	    ...
	    synchronize(function() {
			...
			if ( tests ) {
				...
				for ( var i = 0; i < config.assertions.length; i++ ) {
					...
					li.className = assertion.visual ? "visual" : li.className;
					li.innerHTML = assertion.message || "(no message)";
					ol.appendChild( li );

					if ( assertion.result ) {
						good++;
					} else if(assertion.visual){
						visual++;
					} else {
						bad++;
						config.stats.bad++;
				...
				li.id = "";
				li.className = visual ? "visual" : "pass";
				li.className = bad ? "fail" : li.className;
	...
	visual: function(canvas, msg) {
		msg = escapeHtml(msg||"");
		msg +='<br /><img src="'+canvas.toDataURL()+'"></img>';
		QUnit.log(canvas.toDataURL());
		config.assertions.push({
			visual: true,
			message: msg
		});
